var page_actions = function(){
    ...
    $(".panel-refresh").on("click",function(){
        ...
        panel_refresh(panel);
        if(panel.attr('id') == 'rolePanel' || panel.attr('id') == 'userPanel' || panel.attr('id') == 'makePanel' || panel.attr('id') == 'plateTypePanel'){
            reload();
        }
        setTimeout(function(){
            panel_refresh(panel);
    ...
    $(".lockscreen-box .user_signin").on("click",function(){        
        $(".sign-in").show();
        $(this).remove();
        $(".user").hide().find("img").attr("src", assetURL + "assets/images/users/no-image.jpg");
        $(".user").show();
        return false;
    });
    /* END LOCK SCREEN */
...
function panel_refresh(panel,action,callback){        
    if(!panel.hasClass("panel-refreshing")){
        panel.append('<div class="panel-refresh-layer"><img src="' + assetURL + 'img/loaders/default.gif"/></div>');
...
function pageLoadingFrame(action,ver){    
    ...
    loader['v1'] = '<div class="page-loading-loader"><img src="' + assetURL + 'img/loaders/page-loader.gif"/></div>';
